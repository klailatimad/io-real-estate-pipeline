...
{% set run_date = var('run_date', modules.datetime.date.today().isoformat()) %}

with src as (
  select * from read_csv_auto(
    '../data/raw/io_listings/' || {{ run_date|tojson }} || '/io_listings.csv',
    header=true
  )
)
select
  cast(property_id as int)        as property_id,
